...
def parse_changed_file(raw: object, *, field: str) -> ChangedFile:
...
def parse_conversation_payload(
    raw: object,
    *,
    field: str = "payload",
) -> ConversationPayload:
...
def parse_pull_request_merged_payload(
    raw: object,
    *,
    field: str = "payload",
) -> PullRequestMergedPayload:
    ...
    changed_files_data = _list(data.get("changed_files", []), f"{field}.changed_files")
    return PullRequestMergedPayload(
        task_id=task_id,
        branch=branch,
        ...
        merge_commit_sha=_optional_string(
            data,
            "merge_commit_sha",
            field=field,
            max_length=256,
        ...
        deletions=_optional_non_negative_int(data, "deletions", field=field),
        changed_files=tuple(
            parse_changed_file(item, field=f"{field}.changed_files[{index}]")
            for index, item in enumerate(changed_files_data)
        ),
    )


def parse_inbox_event(raw: object) -> InboxEvent:
    ...
    if event_type == "conversation":
        payload: InboxPayload = parse_conversation_payload(data["payload"])
    elif event_type == "pull_request_merged":
        payload = parse_pull_request_merged_payload(data["payload"])
    else:
        raise InboxValidationError(
...
def _mapping(raw: object, field: str) -> dict[str, object]:
    ...
    if len(data) > MAX_LIST_ITEMS:
        raise InboxValidationError(
            "value_too_large",
            field,
            f"must contain at most {MAX_LIST_ITEMS} keys",
        )
...
def _path_list(value: object, field: str) -> tuple[str, ...]:
    return tuple(
        _relative_path(item, f"{field}[{index}]")
        for index, item in enumerate(_list(value, field))
    )
...
def _source_context(value: object, field: str) -> dict[str, object]:
    if type(value) is not dict:
        raise InboxValidationError("invalid_type", field, "must be an object")
    cloned = _json_value(value, field, depth=0, budget=[0])
