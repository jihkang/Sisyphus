...
def _verification_artifact_path(
    ...
    *,
    ...
    compatibility_path = getattr(outcome, "verify_file", None)
    if compatibility_path is not None:
        return Path(compatibility_path)
    artifact = getattr(outcome, "verify_artifact", None)
    relative_path = getattr(artifact, "relative_path", None)
    if not isinstance(relative_path, str) or not relative_path:
        raise ValueError("verification outcome is missing its artifact locator")
