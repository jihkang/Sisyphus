...
def parse_local_provider_args(
    ...
    *,
    ...
    add_local_provider_arguments(parser)
    try:
    ...
    if unknown:
        raise LocalProviderConfigError(
            f"unsupported local provider argument(s): {' '.join(unknown)}"
        )
...
def local_provider_available(config: LocalProviderConfig) -> bool:
    ...
    try:
        ...
        with request.urlopen(
            http_request,
            timeout=config.availability_timeout_seconds,
        ) as response:
            response.read(1)
        return True
...
def add_local_provider_arguments(parser: argparse.ArgumentParser) -> None:
...
def config_argv_from_namespace(args: argparse.Namespace) -> list[str]:
    values = [
...
def _validate_config(config: LocalProviderConfig) -> None:
    ...
    if config.context_reserve_tokens >= config.context_window_tokens:
        raise LocalProviderConfigError(
            "local provider context reserve must be smaller than the context window"
        )
    ...
    if not config.test_commands:
        raise LocalProviderConfigError(
            "local provider requires at least one operator-configured test command"
        )
...
def _fallback_provider(
    ...
    if fallback == provider:
        raise LocalProviderConfigError(
            "local provider fallback cannot point to the same provider"
        )
    return fallback
...
__all__ = [
    ...
    "add_local_provider_arguments",
    "build_worker_command",
    "config_argv_from_namespace",
