...
def project_feature_task_data(
    ...
    *,
    ...
    feature_id = _feature_id_for(task)
    spec_artifact = _build_spec_artifact(
        task,
        feature_id,
    ...
    implementation_artifact = _build_implementation_artifact(task, feature_id)
    test_artifacts = _build_test_artifacts(task, feature_id)
    execution_receipts = _build_execution_receipts(task, feature_id)
    task_run_refs = _build_task_run_refs(task, execution_receipts)
    ...
    *,
    ...
    return ArtifactRecord(
        ...
        payload={
            "feature_id": feature_id,
            "task_id": str(task["id"]),
            "brief_path": PurePosixPath(brief_path).name,
            "brief_content": brief_content,
            "plan_path": PurePosixPath(plan_path).name,
            "plan_content": plan_content,
        },
    ...
    *,
    ...
    return CompositeArtifactRecord(
        ...
        payload={
            ...
            "workflow_phase": str(task.get("workflow_phase") or ""),
            "verify_status": str(task.get("verify_status") or ""),
            "slot_bindings": encode_feature_change_slot_bindings(slot_bindings),
            "verification_claims": [
                encode_verification_claim(claim) for claim in verification_claims
            ],
        },
        ...
        task_specs=(
            TaskSpecRef(
                ...
                doc_path=(PurePosixPath(str(task["task_dir"])) / str(task["docs"]["plan"])).as_posix(),
            ),
        ),
...
def _input_fingerprint(refs: tuple[ArtifactRef, ...]) -> str:
    payload = json.dumps(
        [encode_artifact_ref(ref) for ref in refs],
        separators=(",", ":"),
        sort_keys=True,
    )
...
__all__ = ["FeatureTaskArtifactProjection", "project_feature_task_data"]
