...
TransitionEvaluator = Callable[[dict, LifecycleAction], TransitionView]
...
def allowed_policy_actions_for(
    task: dict,
    evaluate_transition: TransitionEvaluator,
) -> tuple[str, ...]:
    return tuple(
        name
        for name, spec in sorted(ACTION_REGISTRY.items())
        if spec.allowed_for_policy
        and (
            spec.lifecycle_action is None
            or evaluate_transition(task, spec.lifecycle_action).allowed
        )
    )


def forbidden_policy_actions_for(
    task: dict,
    evaluate_transition: TransitionEvaluator,
