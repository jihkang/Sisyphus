...
def lifecycle_snapshot_from_record(
    task: TaskRecord,
    *,
...
def _map_conformance(task: TaskRecord) -> ConformanceState:
...
def _is_closed(task: Mapping[str, object]) -> bool:
    return str(task.get("status") or "").strip().lower() == "closed" or bool(
        task.get("closed_at")
    )


__all__ = ["lifecycle_snapshot_from_record"]
