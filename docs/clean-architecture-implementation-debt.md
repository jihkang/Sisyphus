...
| CA-06 | High | The infrastructure workflow adapter still calls root `closeout`, `conformance`, and `obligation_runtime` facades; legacy conversation dispatch dynamically loads `daemon`. Planning and spec-validation facade dependencies are closed | Make the workflow adapter depend on inward policy/record functions and existing repository/effect ports; replace dynamic legacy dispatch when creation/daemon orchestration moves inward | Architecture test rejects workflow-adapter-to-facade edges; adapter contract and failure-injection suites pass |
