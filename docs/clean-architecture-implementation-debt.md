...
| `application/use_cases/verification.py` | Split | The verify transaction remains 458 lines; `application/verification_projection.py` owns VERIFY Markdown and template-marker detection, while `application/external_review_verification.py` owns strict evidence revalidation and binding projection. The service still owns one ordered gate/command/evidence/save/event transaction |
