...
| CA-06 | High | The infrastructure workflow adapter still calls root `closeout` and `obligation_runtime` facades; legacy conversation dispatch dynamically loads `daemon`. Planning, spec-validation, and conformance facade dependencies are closed | Replace closeout and obligation calls with application use cases over explicit ports; replace dynamic legacy dispatch when creation/daemon orchestration moves inward | Architecture test rejects workflow-adapter-to-facade edges; adapter contract and failure-injection suites pass |
