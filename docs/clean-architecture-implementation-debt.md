...
| Independent review trust-boundary remediation, round 2 | Implemented; re-review pending | Replaced caller-authored review verdicts with strict envelopes and scope bindings; restored all inbox serialization methods; made review changes invalidate verify/promotion; made Git dirty inspection fail closed with NUL parsing; removed task-directory-wide verify exemptions; made shared run-store parent creation race-safe while retaining exclusive run IDs. Final self-review also made promotion re-inspect evidence and push the exact reviewed SHA. The full 775-test suite passes on Python 3.11-3.14 with 85.1% branch coverage; standard and offline builds plus installed-wheel smoke pass; an immutable-commit independent re-review remains part of CA-11 |
| Independent review trust-boundary remediation, round 3 | Implemented; re-review pending | The review scope now binds the complete document mapping and exact non-colliding service output paths. Verification writes only the reviewed path, reloads and compares latest authority under the task update lock, preserves concurrent mutations, and performs post-output evidence inspection. Closeout re-inspects HEAD/scope/evidence independently of `allow_dirty`. Promotion permits its exact reviewed receipt and resumes a pushed attempt after PR API failure without another push. Focused regressions and the full 785-test suite pass; immutable-commit re-review remains part of CA-11 |
| Independent review concurrency and retry remediation, round 4 | Implemented; re-review pending | Verification preserves gates added concurrently; generated outputs cannot overlap authority, frozen-input, or review paths; Git closeout inspection fails closed without a dirty override bypass; new commits cannot inherit stale pushed state; PR lookup and final receipt repair make external retries idempotent; review scope binds the remote-first base revision and promotion target. The full 805-test suite passes on Python 3.11-3.14 with 85.2% branch coverage; immutable-commit re-review remains part of CA-11 |
| Independent review live-remote and recovery remediation, round 5 | Implemented; re-review pending | Reviewed promotion compares against the persisted pre-attempt SHA, rejects repository redirection, and recovers a commit after state-save failure from workspace HEAD. Review scope queries the live remote base without relying on tracking refs, binds the remote URL digest, and fails closed on remote lookup failure. Promotion control-state saves do not mirror into the worktree, while retry classification excludes only exact task-state and receipt outputs. The full 813-test suite and 194 focused review/security/architecture tests pass at 85.2% branch coverage; immutable-commit re-review remains part of CA-11 |
| Independent review push-destination remediation, round 6 | Implemented; re-review pending | Review scope now binds the canonical set of effective Git push URLs as well as the fetch URL and live base SHA, and fails closed if a configured remote has no resolvable push destination. A regression proves that changing only `remote.origin.pushurl` invalidates review evidence. The full 814-test suite and 195 focused review/security/architecture tests pass on Python 3.11-3.14 with 85.2% branch coverage; immutable-commit re-review remains part of CA-11 |
| Pre-merge repository-identity normalization, round 7 | Implemented; re-review pending | A required pre-merge scope check stopped PR #64 after promotion persisted the same remote-derived repository identity that had been implicit at review time. Scope now binds the effective explicit-or-derived repository identity rather than nullable storage representation, while remote URL and push-destination digests continue to detect real target changes. The full 815-test suite passes on Python 3.11-3.14, 196 review/security/architecture tests pass, and branch coverage is 85.2%; immutable-commit re-review and renewed CI remain part of CA-11 |
...
| --- | --- | --- | --- | --- |
| CA-11 | Release gate | Local coverage, standard/offline builds, and isolated wheel-install smoke pass; immutable review, PR/CI/merge, merge receipt, and merged-main validation remain | Record the strict review against the final commit, run Sisyphus verification, then promote and verify the merged commit | Coverage >=80%, wheel install smoke, Sisyphus verify green, CI green, merge recorded, and tests green on updated `main` |
