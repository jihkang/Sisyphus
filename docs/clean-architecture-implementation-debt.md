...
| --- | --- | --- | --- | --- |
| CA-05 | High | Request, command, status, promotion, daemon/service runtime, task-record, search, and ContextPack traffic now use application/composition paths. Event/metrics, episode/observation, artifact/evidence resources, and some MCP resource projections still import flat root implementations; Evolution/artifact authority and benchmark/eval edges are accounted for separately in CA-07 and CA-09 | Route the remaining interface reads through cohesive query services or pure inward projections; interfaces retain only adaptation, dispatch, trace, and rendering | CLI help/exit/output and MCP tools/resources/schema/trace fixtures remain identical; non-Evolution interfaces have no flat implementation imports |
