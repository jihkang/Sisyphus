...
| --- | --- | --- | --- | --- |
| CA-05 | High | Request/ingest/task-query traffic now uses application services, but planning, verification, closeout, promotion, daemon/runtime, status, and MCP resource handlers still import root implementations such as `state`, `planning`, `audit`, `closeout`, `creation`, and `daemon` | Route the remaining handlers through application command/query services assembled by composition roots; interfaces retain only adaptation, dispatch, trace, and rendering | CLI help/exit/output and MCP tools/resources/schema/trace fixtures remain identical; interfaces have no implementation imports |
