...
| CA-06 | High | Infrastructure workflow/planning adapters and spec validation still call root facades such as `closeout`, `conformance`, `state`, and `strategy`; legacy conversation dispatch dynamically loads `daemon` | Make the remaining adapters depend on inward policy/record functions and existing repository/effect ports; replace dynamic legacy dispatch when creation/daemon orchestration moves inward | Architecture test rejects adapter-to-facade edges; adapter contract and failure-injection suites pass |
...
1. Complete workflow/planning and spec-validation adapter dependencies; remove dynamic daemon dispatch with the creation/daemon use cases.
