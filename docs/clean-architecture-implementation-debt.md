...
| Boundary serialization ownership | Completed | Declared model-owned `to_dict`/`from_dict` and `to_json` implementations were reduced to zero. Artifact, event, episode, search, evaluation, benchmark, action, and provider wire shapes live in explicit context codecs. One compatibility utility restores the previously published methods at outer facades, eliminating per-model duplication while preserving installed-wheel behavior. Architecture guards reject mapping implementations in canonical models and restrict facade calls to that helper |
...
| External LLM review evidence transition | Completed | A human-only CLI/MCP action records an independent verdict through `ExternalReviewService`; the evidence adapter enforces a contained no-follow report, a 1 MiB limit, SHA-256 digest, exact Git HEAD, and no unrelated dirty paths. Verification remains fail-closed until the recorded status is `passed` |
...
| Independent review P1/P2 remediation | Completed | Evolution run storage is rooted in descriptor-relative no-follow operations so ancestor substitution cannot escape the repository; public serialization and package-level repository exports are restored; evolution commands now share bounded timeout/output/process-group execution and persist timeout/truncation evidence. The focused 60-test boundary set and full 755-test suite pass at 85.2% branch coverage |
