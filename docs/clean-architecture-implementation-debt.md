...
| --- | --- | --- | --- | --- |
| CA-05 | High | Request, command, status, promotion, daemon/service runtime, task-record, search, ContextPack, event, metrics, repo-status, lifecycle action, observation, evidence, and episode traffic now use application/composition paths. Artifact task resources still import flat root projection/evaluation implementations; the deeper Evolution/artifact authority split remains tracked in CA-07 | Route artifact resources through explicit query/evaluation boundaries; interfaces retain only adaptation, dispatch, trace, and rendering | CLI help/exit/output and MCP tools/resources/schema/trace fixtures remain identical; non-Evolution interfaces have no flat implementation imports |
