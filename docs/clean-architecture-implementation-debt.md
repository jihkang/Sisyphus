...
- A compatibility shim may contain a module docstring, explicit imports, and a
  ...
  runtime state.
- A stable outer facade may install a formerly public serialization method only
  through `compat.serialization.install_serialization_compat`; the method must
  delegate to an explicit codec and may not define a second wire shape.
...
| Artifact, DSL, snapshot, and execution-policy ownership | Completed | Artifact and DSL value models are domain-owned without declared boundary mapping methods; pure projection, evaluation, obligation, snapshot, and execution-policy decisions are application-owned; explicit codecs own wire shapes; package declaration reads and secure document/snapshot IO are infrastructure-owned. Stable root modules are compatibility-only facades, infrastructure imports only canonical owners, and public legacy methods delegate to the same codecs |
| Boundary serialization ownership | Completed | Declared model-owned `to_dict`/`from_dict` and `to_json` implementations were reduced to zero. Artifact, inbox, event, episode, search, evaluation, benchmark, action, and provider wire shapes live in explicit context codecs. One compatibility utility restores the previously published methods at outer facades, including all four legacy inbox models and their keyword-compatible decoders, while preserving installed-wheel behavior. Architecture guards reject mapping implementations in canonical models and restrict facade calls to that helper |
...
| External LLM review evidence transition | Completed | `ExternalReviewService` accepts only a task ID and strict JSON envelope under the task review-artifact directory. Reviewer, findings, counts, and pass/fail status are content-derived; report/envelope digests, exact HEAD, normalized task/spec/verification-policy scope, and exact dirty paths are checked. Recording invalidates prior verify/promotion state; verify rechecks before and after commands and binds success to the exact evidence. Promotion re-inspects the binding, rejects new workspace changes, and pushes the reviewed commit without staging self-referential evidence. MCP recording requires a non-persisted operator capability |
...
| Support-file mirror containment | Completed | Task support files are mirrored through bounded descriptor-relative reads and atomic no-follow writes. Unsafe document traversal, source-file symlinks, target-directory symlinks, and direct misuse of `SecureWorkspaceFiles` with absolute or parent paths are rejected by focused persistence/workspace regressions |
| Independent review P1/P2 remediation, round 1 | Completed | Evolution run storage is rooted in descriptor-relative no-follow operations so ancestor substitution cannot escape the repository; public serialization and package-level repository exports are restored; evolution commands share bounded timeout/output/process-group execution and persist timeout/truncation evidence |
| Independent review trust-boundary remediation, round 2 | Completed; closed by round 8 | Replaced caller-authored review verdicts with strict envelopes and scope bindings; restored all inbox serialization methods; made review changes invalidate verify/promotion; made Git dirty inspection fail closed with NUL parsing; removed task-directory-wide verify exemptions; made shared run-store parent creation race-safe while retaining exclusive run IDs. Final self-review also made promotion re-inspect evidence and push the exact reviewed SHA. The full 775-test suite passed on Python 3.11-3.14 with 85.1% branch coverage; standard and offline builds plus installed-wheel smoke passed |
| Independent review trust-boundary remediation, round 3 | Completed; closed by round 8 | The review scope binds the complete document mapping and exact non-colliding service output paths. Verification writes only the reviewed path, reloads and compares latest authority under the task update lock, preserves concurrent mutations, and performs post-output evidence inspection. Closeout re-inspects HEAD/scope/evidence independently of `allow_dirty`. Promotion permits its exact reviewed receipt and resumes a pushed attempt after PR API failure without another push. Focused regressions and the full 785-test suite passed |
| Independent review concurrency and retry remediation, round 4 | Completed; closed by round 8 | Verification preserves gates added concurrently; generated outputs cannot overlap authority, frozen-input, or review paths; Git closeout inspection fails closed without a dirty override bypass; new commits cannot inherit stale pushed state; PR lookup and final receipt repair make external retries idempotent; review scope binds the remote-first base revision and promotion target. The full 805-test suite passed on Python 3.11-3.14 with 85.2% branch coverage |
| Independent review live-remote and recovery remediation, round 5 | Completed; closed by round 8 | Reviewed promotion compares against the persisted pre-attempt SHA, rejects repository redirection, and recovers a commit after state-save failure from workspace HEAD. Review scope queries the live remote base without relying on tracking refs, binds the remote URL digest, and fails closed on remote lookup failure. Promotion control-state saves remain in the control repository, while retry classification excludes only exact task-state and receipt outputs. The full 813-test suite and 194 focused review/security/architecture tests passed at 85.2% branch coverage |
| Independent review push-destination remediation, round 6 | Completed; closed by round 8 | Review scope binds the canonical set of effective Git push URLs as well as the fetch URL and live base SHA, and fails closed if a configured remote has no resolvable push destination. A regression proves that changing only `remote.origin.pushurl` invalidates review evidence. The full 814-test suite and 195 focused review/security/architecture tests passed on Python 3.11-3.14 with 85.2% branch coverage |
| Pre-merge repository-identity normalization, round 7 | Completed; closed by round 8 | A required pre-merge scope check stopped PR #64 after promotion persisted the same remote-derived repository identity that had been implicit at review time. Scope now binds the effective explicit-or-derived repository identity rather than nullable storage representation, while remote URL and push-destination digests continue to detect real target changes. The full 815-test suite passed on Python 3.11-3.14, 196 review/security/architecture tests passed, and branch coverage was 85.2% |
| Immutable independent review and release closure, round 8 | Completed | The final immutable commit `89c2102` passed independent review with no P0-P2 findings and 96 directly affected tests. Sisyphus verification and conformance passed, all six renewed CI jobs passed, PR #64 squash-merged as `6330966`, the merge receipt was recorded, and 815 tests plus standard/offline package and installed-wheel checks passed on updated `main` |
...
| `application/use_cases/verification.py` | Split | The service owns the ordered snapshot/gate/command/output/atomic-commit/recheck/event transaction. `application/verification_projection.py` owns VERIFY Markdown and template-marker detection, `application/external_review_verification.py` owns strict evidence revalidation and binding projection, and `application/review_scope.py` owns canonical reviewed scope and output-path rules. Further splitting the transaction would distribute its concurrency invariant across services |
...
compatibility shims, `0` Medium implementation items, and `0` release gates. The
...
No implementation item or release gate remains open. The two entries under
**Accepted Domain Dependency Exceptions** are the complete remaining debt list.

## Completed Release Gate

| ID | Status | Closure evidence |
| --- | --- | --- |
| CA-11 | Completed | Immutable round-8 review passed; coverage was 85.2%; standard/offline builds and installed-wheel smoke passed; Sisyphus verify and conformance were green; all six CI jobs passed; PR #64 merged and was recorded; 815 tests passed on updated `main` |

## Execution Order

1. Retire each import-only shim only after its documented compatibility window
   and repository-wide consumer check pass.

No release-blocking migration step remains. Docker service separation,
Hermes-backed agent evolution, GEPA, and the real 30.5B benchmark belong to the
separate Sisyphus Harness workspace and are not implementation debt in this
