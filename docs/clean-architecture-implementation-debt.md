...
| --- | --- | --- | --- | --- |
| CA-05 | High | Request, command, status, promotion, daemon/service runtime, task-record, search, ContextPack, event, metrics, and repo-status traffic now use application/composition paths. Episode/observation plus artifact/evidence task resources still import flat root implementations; Evolution/artifact authority and benchmark/eval edges are accounted for separately in CA-07 and CA-09 | Route the remaining interface reads and trace writes through cohesive query services, append-only ports, or pure inward projections; interfaces retain only adaptation, dispatch, trace, and rendering | CLI help/exit/output and MCP tools/resources/schema/trace fixtures remain identical; non-Evolution interfaces have no flat implementation imports |
