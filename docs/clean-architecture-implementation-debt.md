...
| --- | --- | --- | --- | --- |
| CA-07 | High | Evolution dataset/operator/receipt/verification paths now use read-only query, request-only follow-up, event, and append-only run-artifact ports. Remaining authority leakage is concentrated in `evolution/harness.py`, whose Sisyphus/worktree evaluators still request tasks, approve plans, freeze specs, run providers/subprocesses, and write evaluation files directly. Artifact query, obligation, and search adapters also still import flat artifact projection/evaluator/snapshot/DSL policy modules | Move evaluation execution under a Control-owned application boundary while Evolution retains plans, metrics, and recommendations only. Expose canonical artifact read/evaluation/execution-policy contracts and move artifact/DSL policy under explicit application/domain owners | Authority tests prove forbidden actions are unreachable from Evolution; baseline/candidate/obligation artifacts and existing MCP/CLI projections remain compatible |
