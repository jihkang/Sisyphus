...
| --- | --- | --- | --- | --- |
| CA-05 | High | Request, command, status, promotion, daemon/service runtime, task-record, search, ContextPack, event, metrics, repo-status, lifecycle action, observation, and evidence traffic now use application/composition paths. Episode trace and artifact task resources still import flat root implementations; Evolution/artifact authority and benchmark/eval edges are accounted for separately in CA-07 and CA-09 | Route episode reads/writes through an append-only trace port and artifact resources through explicit query/evaluation boundaries; interfaces retain only adaptation, dispatch, trace, and rendering | CLI help/exit/output and MCP tools/resources/schema/trace fixtures remain identical; non-Evolution interfaces have no flat implementation imports |
