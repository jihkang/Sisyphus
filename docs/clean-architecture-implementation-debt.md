...
| --- | --- | --- | --- | --- |
| CA-05 | High | Request, planning, verification, closeout, promotion, daemon/service runtime, status, and task-record query traffic now use application/composition paths. Search/context, event/metrics, episode/observation, and some MCP resource projections still import flat root implementations; Evolution/artifact and benchmark/eval edges are accounted for separately in CA-07 and CA-09 | Route the remaining interface reads through cohesive query services or pure inward projections; interfaces retain only adaptation, dispatch, trace, and rendering | CLI help/exit/output and MCP tools/resources/schema/trace fixtures remain identical; non-Evolution interfaces have no flat implementation imports |
