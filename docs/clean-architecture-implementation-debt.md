...
| --- | --- | --- | --- | --- |
| CA-04 | High | `daemon.py` is 902 lines and `creation.py` still coordinate queue, task state, worktrees, providers, verification, events, and promotion through root modules. Closeout orchestration is closed | Introduce application use cases for creation and daemon event handling with narrow repository/effect ports; keep side-effect order explicit | Event replay, retry, gate, partial-failure, and end-to-end workflow characterization tests pass |
...
| CA-06 | High | The infrastructure workflow adapter has exactly one guarded root dependency, `obligation_runtime`; legacy conversation dispatch dynamically loads `daemon`. Planning, spec-validation, conformance, and closeout facade dependencies are closed | Replace obligation calls with an application use case over explicit ports; replace dynamic legacy dispatch when creation/daemon orchestration moves inward | Architecture test requires exactly the one documented edge until removal; adapter contract and failure-injection suites pass |
...
1. Complete the remaining workflow adapter dependencies and remove dynamic daemon dispatch with the creation/daemon use cases.
