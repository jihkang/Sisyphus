...
1. Align the replay task to the current root-adopted baseline so the contract cleanup lands in the canonical `sisyphus` tree rather than an older task worktree snapshot.
2. Inventory the current evolution code surface and the evolution-related docs to determine which names already exist and which names are still only proposed.
3. Define or refine the minimum contract vocabulary that the next implementation slices will depend on without introducing implied runtime behavior.
4. Rewrite the evolution docs so they explicitly separate current implementation from future work.
5. Add or adjust tests and task docs so the contract names and doc claims stay locked to the real codebase state.
