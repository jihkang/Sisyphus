...
- Task ID: `TF-20260419-feature-align-evolution-contract-and-doc-reality-root-replay`
- Type: `feature`
- Slug: `align-evolution-contract-and-doc-reality-root-replay`
- Branch: `feat/align-evolution-contract-and-doc-reality-root-replay`

## Problem

- `TF-20260418-feature-align-evolution-contract-and-doc-reality` already defined and verified the contract/doc alignment slice, but that task is blocked only because its stale task worktree predates the current root source-of-truth.
- The current root worktree now carries the active `taskflow` -> `sisyphus` migration baseline, so the contract cleanup must be replayed on top of that adopted baseline rather than resumed in the old worktree.
- This follow-up task must preserve the original narrow boundary: contract vocabulary and documentation alignment only.

## Desired Outcome

- The evolution slice has a stable minimum contract vocabulary for the next implementation steps on top of the current root-adopted baseline.
