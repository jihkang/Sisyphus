...
1. Align the replay task to the current root-adopted baseline so the authority-boundary wording is frozen in the canonical `sisyphus` tree rather than an older task worktree snapshot.
2. Re-read the existing architecture and evolution docs and collect every current sentence that assigns authority to `evolution`, `Sisyphus`, the harness executor, or read-only orchestration.
3. Replace ambiguous or descriptive wording with normative contract language.
...
4. Make the boundary visible in both the general architecture doc and the evolution-specific doc so later implementation tasks inherit one consistent contract.
5. Update task docs and verification notes to reflect the final language and review scope.
