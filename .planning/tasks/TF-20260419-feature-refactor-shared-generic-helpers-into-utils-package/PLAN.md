...
1. Inspect the current live helpers and split them into two buckets:
...
2. Convert `src/sisyphus/utils.py` into a `src/sisyphus/utils/` package while preserving current import ergonomics through `__init__.py` re-exports.
3. Add shared utility modules for the extracted generic helpers.
...
4. Update live callers to import the shared helpers from the new utils package and remove duplicated local implementations where the behavior is genuinely shared.
5. Add focused regression tests for:
   - the shared utils package behavior
   - touched modules such as `sisyphus.mcp_core` and `sisyphus.evolution.dataset`
6. Update task docs and verification notes to record the final helper boundaries and test coverage.
