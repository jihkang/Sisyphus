...
1. Align the replay task to the current root-adopted baseline so the stage machine is defined in the canonical `sisyphus` tree rather than an older task worktree snapshot.
2. Review the current evolution helpers and identify the actual transition points that already exist conceptually: run planning, dataset build, harness planning, constraints, fitness, and report.
3. Define the initial read-only stage sequence and document later extension stages without implementing them yet.
4. For each initial stage, define the expected input contract, produced output, invariants, and failure shape.
5. Add or adjust tests and docs so the stage names and failure model stay stable for the orchestration task.
