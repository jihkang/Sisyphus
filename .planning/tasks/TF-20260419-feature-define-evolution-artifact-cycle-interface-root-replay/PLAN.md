...
1. Align the replay task to the current root-adopted baseline so the artifact-cycle interface is defined in the canonical `sisyphus` tree rather than an older task worktree snapshot.
2. Review the current evolution modules and existing artifact-centric architecture notes to determine which artifact kinds are needed for the next execution slices.
3. Define the minimum artifact kinds for the evolution vertical slice and the minimum fields each artifact must expose for reconstruction.
4. Distinguish artifact ownership between evolution-produced planning or evaluation artifacts and Sisyphus-authoritative execution or verification artifacts.
5. Update tests and docs so the artifact interface is explicit before orchestration or bridge work begins.
