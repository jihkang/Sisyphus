...
1. Add a dedicated artifact model module in the canonical `sisyphus` package for durable artifact records.
   - `ArtifactRef`
   - `TaskSpecRef`
   - `TaskRunRef`
   - `ArtifactLineage`
...
2. Define a stable serialization contract.
...
3. Align the initial model to the `FeatureChangeArtifact` protocol without implementing the protocol-specific slots yet.
   - preserve artifact refs, task refs, lineage, and invariant results
   - keep slot binding, verification claims, promotion decisions, and invalidation logic out of scope for this slice
4. Add focused regression coverage and update task docs to reflect the actual model boundary.
